...
const InterruptFlags = packed struct {
    ...
    fn check(self: InterruptFlags, int: Interrupt) bool {
        return switch (int) {
            .Joypad => self.joypad,
            .Serial => self.serial,
            .Timer => self.timer,
    ...
    fn set(self: *InterruptFlags, int: Interrupt, value: bool) void {
        switch (int) {
...
pub fn CPU(comptime WriterType: type) type {
    return struct {
        const Self = @This();
        ...
        // writer: std.io.BufferedWriter(4096, @TypeOf(std.io.getStdOut())) = std.io.bufferedWriter(std.io.getStdOut()),
        writer: WriterType,
        ...
        pub fn init(
            ...
            return Self{ .table = try opcodes.Opcodes.init(allocator), .writer = writer };
        }
        ...
        pub fn doctorLog(self: *Self) void {
        ...
        /// Set the Self Flag register to the passed value
        pub fn set_flag(self: *Self, flag: Flag) void {
        ...
        pub fn flag_state(self: *Self) *Flags {
            return @ptrCast(&self.registers[6]);
        }

        // An interrupt has been requested, and now we must handle it
        pub fn handle_interrupt(self: *Self, interrupt: Interrupt) void {
            const IEnable: InterruptFlags = @bitCast(self.read(u8, IE));
            var IFlags: InterruptFlags = @bitCast(self.read(u8, IF));
        ...
        pub fn poll_interrupts(self: *Self) ?Interrupt {
            if (!self.ime) return null;

            const IFlags: InterruptFlags = @bitCast(self.read(u8, IF));
            const IEnable: InterruptFlags = @bitCast(self.read(u8, IE));
        ...
        pub fn read(self: *Self, comptime T: type, from: anytype) T {
        ...
        pub fn write(self: *Self, comptime T: type, dest: anytype, src: T) void {
            switch (@TypeOf(dest)) {
                ...
                u16 => {
                    switch (dest) {
                        0xFF02 => if (src == 0x81) self.serialTransfer(),
                        else => {
                            ...
                            // transfer requested
                            // if (dest == IF) std.log.warn("Writing to IF: {any}", .{src});
                            // if (dest == IE) std.log.warn("Writing to IE: {any}", .{src});
                        },
                    }
                },
        ...
        pub fn address_ptr(self: *Self, addr: opcodes.Address) u16 {
            return switch (addr) {
                .bc, .de, .hl => ret: {
        ...
        // TODO: test this
        pub fn pc_read(self: *Self, comptime T: type) T {
        ...
        // step the Self forward by one instruction
        ...
        pub fn decode(self: *Self, opcode: u8) Instruction {
        ...
        pub fn execute(self: *Self, instr: Instruction) usize {
            return switch (instr) {
                .Nop => 4,
        ...
        pub fn stackPush(self: *Self, r16: Register16) void {
            self.sp -= 2;
        ...
        pub fn stackPop(self: *Self, r16: Register16) void {
            defer self.sp += 2;
        ...
        pub fn executeStackOp(self: *Self, op: StackOp) usize {
            switch (op.op) {
        ...
        pub fn executeLoad(self: *Self, op: Load) usize {
            // op.log();
            switch (op.src) {
                // 8 bit load
                .r8, .imm8, .address => {
        ...
        pub fn fetchLoadData(self: *Self, comptime T: type, operand: LoadOperand) T {
        ...
        pub fn executeAdd(self: *Self, add_instr: Add) usize {
        ...
        fn executeALUOp(self: *Self, instr: ByteArithmetic) usize {
        ...
        fn executeInc(self: *Self, inc_instr: Increment) usize {
        ...
        fn executeDec(self: *Self, dec_instr: Decrement) usize {
        ...
        fn executeJump(self: *Self, jmp: Jump) usize {
            ...
            if (Self.check_condition(result.cc, self.flag_state().*)) {
                self.jump(result.addr);
                return jmp.cycles[0];
            } else return jmp.cycles[1];
        }

        fn call(self: *Self, addr: u16) void {
            self.stackPush(Register16.pc);
            self.jump(addr);
        }

        fn executeCall(self: *Self, fn_call: Call) usize {
            const addr = self.pc_read(u16);

            // if the condition is met, then flow through the switch statement,
            // otherwise return the relevant cycle count
            if (Self.check_condition(fn_call.condition, self.flag_state().*)) {
                self.call(addr);
                return fn_call.cycles[0];
            } else return fn_call.cycles[1];
        }

        fn executeRst(self: *Self, rst: Reset) usize {
            self.call(rst.addr);
            return rst.cycles;
        }

        fn executeReturn(self: *Self, fn_ret: Return) usize {
            // if the condition is met, then flow through the switch statement,
            // otherwise return the relevant cycle count
            if (Self.check_condition(fn_ret.condition, self.flag_state().*)) {
                self.stackPop(Register16.pc);
                return fn_ret.cycles[0];
            } else return fn_ret.cycles[1];
        }

        fn executeInterruptControl(self: *Self, intr: InterruptControl) usize {
        ...
        fn acc_and(self: *Self, rhs: u8) Flags {
        ...
        fn acc_or(self: *Self, rhs: u8) Flags {
            var acc = self.read(u8, Register8.a);
            acc |= rhs;
            self.write(u8, Register8.a, acc);
        ...
        fn acc_xor(self: *Self, rhs: u8) Flags {
            var acc = self.read(u8, Register8.a);
            acc ^= rhs;
            self.write(u8, Register8.a, acc);
        ...
        /// the regular Self.add() function
        /// T is the type we're operating on & dest/src are readable/writeable types
        pub fn genericAdd(self: *Self, comptime T: type, dest: anytype, src: T) void {
            const lhs = self.read(T, dest);
            switch (T) {
                u8 => {
                    const result = Self.add(T, lhs, src);
        ...
        pub fn add_sp_e8(self: *Self) u16 {
            const e8 = self.pc_read(u8);
            const lsb: u8 = @truncate(self.sp);
            const flags = Self.add(u8, lsb, e8).@"1";
        ...
        pub fn adc(self: *Self, rhs: u8) void {
            const acc = self.read(u8, Register8.a);

            const partial = Self.add(u8, acc, rhs);
            const partial2 = Self.add(u8, partial.@"0", @as(u8, self.flag_state().carry));

            self.write(u8, Register8.a, partial2.@"0");
            // TODO: Redundant zero flag operations
            self.flag_state().* = Self.join_flags(partial.@"1", partial2.@"1");
        ...
        /// Add to the A register, the carry flag, and the associated data
        pub fn sbc(self: *Self, rhs: u8) Flags {
            const acc = self.read(u8, Register8.a);

            const partial = Self.sub(acc, rhs);
            const partial2 = Self.sub(partial.@"0", @as(u8, self.flag_state().carry));

            self.write(u8, Register8.a, partial2.@"0");
            var flags = Self.join_flags(partial.@"1", partial2.@"1");
        ...
        pub fn inc(self: *Self, comptime T: type, dest: anytype) void {
            const value = self.read(T, dest);
            const result = Self.add(T, value, 1);
            self.write(T, dest, result.@"0");
            if (T == u8) {
        ...
        pub fn dec(self: *Self, comptime T: type, dest: anytype) void {
            var value = self.read(T, dest);
            switch (T) {
                u16 => value -%= 1,
                u8 => {
                    const result = Self.sub(value, 1);
                    value = result.@"0";
                    self.set_flag(.{ .half_carry = result.@"1".half_carry });
                    self.set_flag(.{ .zero = result.@"1".zero });
                    self.set_flag(.{ .subtraction = 1 });
        ...
        // TODO: Cleanup?
        pub fn executeRotation(self: *Self, rot: Rotation) usize {
        ...
        pub fn executeShift(self: *Self, shift: Shift) usize {
        ...
        pub fn executeSwap(self: *Self, swap: Swap) usize {
        ...
        pub fn executeBitOp(self: *Self, bop: BitOperation) usize {
        ...
        pub fn serialTransfer(self: *Self) void {
            ...
            // transfer requested
            _ = self.writer.write(&.{self.read(u8, SB)}) catch unreachable;
            self.write(u8, SC, @as(u8, 0x7F));
            // }
        }
    };
}
